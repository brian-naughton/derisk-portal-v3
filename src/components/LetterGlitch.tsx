...
const LetterGlitch = ({
  glitchColors = ['#0d2b52', '#000515', '#4f8fe6'],
  glitchSpeed = 50,
  centerVignette = true,
  outerVignette = false,
  smooth = true,
  ...
  const getRandomChar = () =>
    contractChars[Math.floor(Math.random() * contractChars.length)];
  ...
  const updateLetters = () => {
    ...
    const updateCount = Math.max(1, Math.floor(letters.current.length * (0.1 * (1 - progress))));
